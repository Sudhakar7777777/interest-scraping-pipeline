...
# Function to process the tenure text in Python (Python-heavy approach)
...
async def scrape_interest_rates_optimized():
    ...
    # Launch the browser (headless mode is on by default)
    ...
    await page.goto(url, {'waitUntil': 'networkidle2'})  # Ensure page is fully loaded

    # Wait for the table to appear (simplified selector)
    print("Waiting for the table to appear...")
    try:
        await page.waitForSelector('#fcnr-interest-rate table', {'timeout': 30000})  # 30s timeout
    ...
    # Extract the table data with the raw structure (minimal JS code)
